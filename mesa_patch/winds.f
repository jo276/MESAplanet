...
! ***********************************************************************
      ...
      real(dp) function eval_stern51_wind(s, L, M, R, Teff, X, Y, Z) result(dms)
         ...
         if (xhs2 > 0.3d0) then ! still have H in envelope
            ...
            if (xhc < 1d-6) then ! center h gone
               logr = log10_cr(R/Rsun)
               logl = log10_cr(L/Lsun)
               logm = log10_cr(M/Msun)
               logz = log10_cr(znow/0.02)
            ...
            dms = dms*pow_cr(znow/0.02,0.69d0)
      ...
      subroutine wr_hamann(xsl,xhs,znow,dms)
         ...
         include 'formats'
         xlogl = log10_cr(xsl/Lsun)
         logz = log10_cr(max(1d-20,znow)/0.02)
         if (xlogl > 4.5) then
      ...
      subroutine eval_super_eddington_wind(s, L, M, R, ierr)
         ...
         write(*,'(a60,i12,992f12.5)') 'super eddington wind: lg_Mdot, L/Ledd', &
            s% model_number, log10_cr(s% super_eddington_wind_mdot/(Msun/secyer)), L/Ledd
      ...
      real(dp) function eval_nova_wind(s, L_surf, R, Teff, ierr) ! value in Msun/year
         ...
         if (R >= roche_lobe_radius) then
            ...
            write(*,1) 'nova RLO log mdot', log10_cr(s% nova_wind_eta*mdot/(Msun/secyer))
         else ! eqn 23 of Kato and Hachisu, ApJ 437:802-826, 1994
            mdot = exp10_cr(1.49d0*log10_cr(Teff/1d5) + s% nova_wind_b) ! in gm per second
            write(*,1) 'nova wind log mdot', log10_cr(s% nova_wind_eta*mdot/(Msun/secyer))
         end if
      ...
      real(dp) function eval_rlo_wind(s, L_surf, R, Teff, xfer_ratio, ierr) ! value in Msun/year
         ...
         if (ratio < 1) then
            ...
            xfer_ratio = 0.5d0*(1 - cospi_cr(xfer_ratio))
            return
         end if
         mdot = s% rlo_wind_base_mdot* &
            exp_cr(min(6*ln10,(R - roche_lobe_radius)/s% rlo_wind_scale_height))
         eval_rlo_wind = s% rlo_wind_eta*mdot ! Msun/year
         
         write(*,1) 'log rlo mdot Msun/yr, R/R_L', log10_cr(eval_rlo_wind), &
            R/roche_lobe_radius
         if (.false. .and. (R - roche_lobe_radius)/s% rlo_wind_scale_height > 100) then
            ...
            write(*,1) 'R', R
            write(*,1) 'exp_cr(s% xh(s% i_lnR,1))/Rsun', &
               exp_cr(s% xh(s% i_lnR,1))/Rsun
      ...
      subroutine set_mdot(s, L_phot, M_phot, T_phot, ierr)
         ...
         if (s% super_eddington_wind_mdot > wind_mdot) then
            ...
            if (dbg) write(*,1) 'super eddington wind mdot', &
               log10_cr(s% super_eddington_wind_mdot/(Msun/secyer))
         end if
            ...
            if (total_H > s% remove_H_wind_H_mass_limit) then
               if (s% remove_H_wind_mdot*Msun/secyer > wind_mdot) then
                  ...
                  if (dbg) write(*,1) 'use remove H wind mdot', &
                     log10_cr(s% remove_H_wind_mdot/(Msun/secyer))
               end if
            end if
         end if
         ...
         if (s% flash_wind_mdot > 0) then
            ...
            if (s% doing_flash_wind .and. s% flash_wind_mdot*Msun/secyer > wind_mdot) then
               if (R1/Rsun < s% flash_wind_declines) then
                  ...
                  s% flash_wind_mdot = &
                     s% flash_wind_mdot*pow_cr(2d0,(R1/Rsun)/s% flash_wind_starts - 1d0)
               end if
         ...
         if (s% doing_rlo_wind .and. mdot > wind_mdot) then
            if (dbg) write(*,1) 's% doing_rlo_wind mdot', log10_cr(mdot/(Msun/secyer))
            wind_mdot = mdot
         end if
         ...
         if (using_wind_scheme_mdot) then
            ...
            if (s% dt > 0 .and. s% dt < s% mass_change_full_on_dt) then
               ...
               !log_dtyr = log10_cr(s% dt/secyer)
               !log_dtyr_full_on = log10_cr(s% mass_change_full_on_dt/secyer)
               !log_dtyr_full_off = log10_cr(s% mass_change_full_off_dt/secyer)
         ...
         if (dbg) then
            write(*,1) 'final lg s% mstar_dot/(Msun/secyer)', safe_log10_cr(abs(s% mstar_dot/(Msun/secyer)))
            write(*,*)
         end if
         ...
         subroutine rotation_enhancement(ierr)
            use star_utils, only: eval_kh_timescale, new_eval_kh_timescale
            integer, intent(out) :: ierr
            ...
            if (.false. .and. s% v_div_v_crit_avg_surf > 0.99d0) then
               ...
               enhancement = pow_cr(max(1d-22, 1d0 - s% v_div_v_crit_avg_surf), -s% mdot_omega_power)
            ...
            if (wind_mdot*enhancement < wind_mdot_lim) then
               ...
               if (.false.) write(*,2) 'wind_mdot = wind_mdot*enhancement', &
                  ...
                  log10_cr(wind_mdot/(Msun/secyer)), log10_cr(wind_mdot_lim/(Msun/secyer))
            else
               enhancement = wind_mdot_lim/wind_mdot
               wind_mdot = wind_mdot_lim
               if (.false.) write(*,2) 'wind_mdot = wind_mdot_lim', &
                  s% model_number, log10_cr(wind_mdot/(Msun/secyer))
            end if
         ...
         subroutine eval_Kudritzki_wind(w)
            ...
            if (dbg) write(*,*) 'lg eval_Kudritzki_wind', log10_cr(w)
         ...
         subroutine eval_lowT_Dutch(w)
            ...
            if (s% Dutch_wind_lowT_scheme == 'de Jager') then
               ...
               if (dbg) write(*,1) 'Dutch_wind = de Jager', log10_cr(wind), T1, T_low, T_high
            else if (s% Dutch_wind_lowT_scheme == 'van Loon') then
               call eval_van_Loon_wind(w)
               if (dbg) write(*,1) 'Dutch_wind = van Loon', log10_cr(wind), T1, T_low, T_high
            else if (s% Dutch_wind_lowT_scheme == 'Nieuwenhuijzen') then
               call eval_Nieuwenhuijzen_wind(w)
               if (dbg) write(*,1) 'Dutch_wind = Nieuwenhuijzen', log10_cr(wind), T1, T_low, T_high
            else
